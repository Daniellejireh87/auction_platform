...
async function createAuction() {
    ...
    const res = await fetch('http://localhost:8012/auctions', {
        ...
        body: JSON.stringify({ title, description, starting_price, ends_at })
    });
...
async function loadAuctions() {
    ...
    auctions.forEach(auction => {
        ...
        div.innerHTML = `
            ...
            <p>Statut : ${auction.status}</p>
            <button onclick="deleteAuction('${auction.id}')">Supprimer</button>
        `;
...
async function deleteAuction(id) {
    const res = await fetch(`http://localhost:8012/auctions/${id}`, {
        method: 'DELETE',
        headers: {
            'Authorization': 'Bearer ' + localStorage.getItem('token')
        }
